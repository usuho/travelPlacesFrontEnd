...
function isUnauthorizedError(err) {
  return !!(err && (err.code === 'UNAUTHORIZED' || err.status === 401));
}

function makeUnauthorizedError(url, responseText = '') {
  const err = new Error('Unauthorized');
  err.code = 'UNAUTHORIZED';
  err.status = 401;
  err.url = url;
  if (responseText) err.details = responseText.slice(0, 200);
  return err;
}

async function fetchWithFallback(path, options, behavior = {}) {
  const onUnauthorized = behavior && behavior.onUnauthorized ? behavior.onUnauthorized : 'logout'; // logout | throw
  ...
  for (const base of bases) {
    ...
    try {
      ...
      if (resp && resp.status === 401) {
        ...
        throw makeUnauthorizedError(url, text);
      }
...
export async function pullUserDataFromServer(options = {}) {
  if (!isAuthenticated()) return null;
  const onUnauthorized = options && options.onUnauthorized ? options.onUnauthorized : 'logout';
  setSyncUsername((getAuthUser() && getAuthUser().username) || '');
  setStatus('syncing', '');
  try {
    const resp = await fetchWithFallback('/api/user/data', { cache: 'no-store' }, { onUnauthorized });
